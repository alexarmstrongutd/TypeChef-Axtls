...
extern SSL * ssl_client_new(SSL_CTX *ssl_ctx, int client_fd, const signed char session_id[], signed char sess_id_size, SSL_EXTENSIONS* ssl_ext);
 ...
 */
extern int ssl_write(SSL *ssl, const signed char out_data[], int out_len);
extern SSL * ssl_find(SSL_CTX *ssl_ctx, int client_fd);
extern const unsigned char * ssl_get_session_id(const SSL *ssl);
extern signed char ssl_get_session_id_size(const SSL *ssl);
extern signed char ssl_get_cipher_id(const SSL *ssl);
...
extern int ssl_obj_memory_load(SSL_CTX *ssl_ctx, int obj_type, const signed char data[], int len, const char *password);
